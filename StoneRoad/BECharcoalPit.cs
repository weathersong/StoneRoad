...
namespace StoneRoad
{
	public class BECharcoalPit : BlockEntity
	{
		...
		public override void Initialize(ICoreAPI api)
		{
			...
			{
				BurnHours = srMod.Config.CharcoalPitBurnHours;
			}
		...
		void ConvertPit()
		{
			srMod.LogWarn($"Charcoal pit is converting...");
					...
					if (BlockFirepit.IsFirewoodPile(Api.World, lpos))  //test for the possibility someone had contiguous firewood both above and below a soil block for example
					{
						if (charCoalQuantity > 0)
						{
							...
							if (isAged)
							{
								...
								Block bedBlock = Api.World.BlockAccessor.GetBlock(bedPos); // returns Air if invalid
								// no bedding = poor charcoal
								if (bedBlock.BlockId == coalPileId)
								{
									BlockEntityCoalPile beCoalPile = Api.World.BlockAccessor.GetBlockEntity(bedPos) as BlockEntityCoalPile;
									if (beCoalPile != null && beCoalPile.inventory.Count > 0) // in case some mod adds broken coalpile or something...
									{
										string coalItemPath = beCoalPile.inventory[0].Itemstack.Collectible.Code.Path;
										// poor bedding = standard (vanilla) quality charcoal
										if (coalItemPath == "charcoal-poor")
		...
		BlockPos FindHoleInPit()
		{
			...
			while (bfsQueue.Count > 0)
			{
				...
				foreach (BlockFacing facing in BlockFacing.ALLFACES)
				{
					...
					{
						srMod.LogWarn($"Charcoal pit found a hole at: {bpos}, BlockId {nBlock.BlockId}, CodePath {nBlock.Code.Path}.");
						return bpos;
					}
		...
		public override bool OnTesselation(ITerrainMeshPool mesher, ITesselatorAPI tessThreadTesselator)
		{
			if (!lit)
			{
				MeshData litCharcoalMesh = ObjectCacheUtil.GetOrCreate(Api, "litCharcoalMesh", () =>
				{
					...
					tess.TesselateShape(Block, Vintagestory.API.Common.Shape.TryGet(Api, "shapes/block/wood/firepit/cold-normal.json"), out mesh);
